...
def parse_args():
    ...
    parser.add_argument("--seed", type=int, default=12345, help="the number of demonstration")
...
class GLUETask(object):
    ...
    def __init__(self, task, train_file, valid_file, test_file):
        ...
        if task in ['mr', 'cr', 'SST-2', 'sst-5', 'subj', 'trec']:
        ...
        elif task in ['cb', 'mrpc', 'qnli', 'qqp', 'rte']:
...
def get_basic_inputs(inputs, task):
    if task in ['mr', 'cr', 'SST-2', 'sst-5', 'subj', 'TREC']:
        ...
        new_inputs = [ input_str[0] + ' </s> ' + input_str[1] + ' </s> ' + input_str[2] for input_str in inputs]

    return new_inputs

def get_prompt_inputs(inputs, labels, task, tindex, verbalizers):
    new_inputs = []
    for input_str, label in zip(inputs, labels):
        new_input_str = get_prompt_str(input_str, label, task, tindex, verbalizers)
...
def get_prompt_demon_inputs(
        ...
        task, tindex, num_label, num_demon, isSameData=False):
    none_verbalizers = ['<mask>'] * num_label
    # get prompt-data for inputs
    new_prompt_inputs = get_prompt_inputs(inputs, labels, task, tindex, none_verbalizers)
                    ...
                    if train_labels[rid] == label_index:
                        if rid in used_ids:
                            ...
                            if total_sum + demon_len < 512:
                                new_inputs[cur_index] += train_data_with_prompt[rid] + '</s>'
                                total_sum += demon_len
                                used_ids.add(rid)
                                find_id = True
                                break
...
def main(args):
    ...
    if args.mode == 0:
        new_train_inputs = get_basic_inputs(data.train_inputs, data.task)
        new_valid_inputs = get_basic_inputs(data.valid_inputs, data.task)
        new_test_inputs = get_basic_inputs(data.test_inputs, data.task)
        ...
        none_verbalizers = ['<mask>'] * args.num_label
        
        for tindex in range(args.tindex):
            new_train_inputs = get_prompt_inputs(data.train_inputs, data.train_labels, data.task, tindex, none_verbalizers)
            new_valid_inputs = get_prompt_inputs(data.valid_inputs, data.valid_labels, data.task, tindex, none_verbalizers)
            new_test_inputs = get_prompt_inputs(data.test_inputs, data.test_labels, data.task, tindex, none_verbalizers)
        ...
        for tindex in range(args.tindex):
            ...
            train_data_with_prompt = get_prompt_inputs(data.train_inputs, data.train_labels, data.task, tindex, verbalizers)
            
            new_train_inputs = get_prompt_demon_inputs(train_data_with_prompt, data.train_labels, \
                    data.train_inputs, data.train_labels, data.task, tindex, args.num_label, args.num_demon_train)
            new_valid_inputs = get_prompt_demon_inputs(train_data_with_prompt, data.train_labels, \
                    data.valid_inputs, data.valid_labels, data.task, tindex, args.num_label, args.num_demon_test)
            new_test_inputs = get_prompt_demon_inputs(train_data_with_prompt, data.train_labels, \
                    data.test_inputs, data.test_labels, data.task, tindex, args.num_label, args.num_demon_test)
